...
LiquidCrystal_I2C lcd_3(0x22,16,2);
...
void setup() {
  ...
  wdt_disable(); // деактивация Watchdog для запуска приложения
  
  Serial.begin(9600);
  BTSerial.begin(9600); 

  wdt_enable (WDTO_8S);  // запуск таймера (8 сек)
  BTSerial.println("Watchdog enabled.");  
  ...
  if(digitalRead(4)==HIGH){
    ...
    parse = true;
    lcd.print("Parsing was succesfull");
...
void lcd_3_Init(){
  BTSerial.println("lcd_3_Init();");  
  lcd_3.init();
  //lcd_3.backlight();
  lcd_3.setCursor(3,1);
  lcd_3.print("TEST MODE");
} 
